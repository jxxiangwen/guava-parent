...
// 1、RateLimiter的记忆非常浅，只能记住上一次被允许请求的时间。
// 2、RateLimiter每次请求时都会去更新剩余的令牌数(storedPermits)
// 和下一次被允许请求的时间nextFreeTicketMicros，以及刷新现在还需要的令牌数(freshPermits);
// 3、若过去很长一段时间没有请求，RateLimiter就会产生剩余的令牌(storedPermits)，且对多能剩余的令牌数为maxPermists;
// ADVERTISEMENT
// 4、假若对于每秒产生一个令牌的RateLimiter而言，若在初始状态（既是还没有被使用），
// 过来一个很大的请求比如acquire(50)，RateLimiter不会阻塞50秒，而是先立即执行，计算下一次请求被允许的时间。
// 5、不同子类对限流的处理方法不同，SmoothWarmingUp类允许用户自定义一个热身时间，
// 当storedPermits>0时，产生令牌的速率会下降。具体的速率是通过计算直线方式斜率得到的。
// 而SmoothBursty类型的RateLimiter当storedPermits>0时，就会加速去使用过去剩余的令牌。
@ThreadSafe
@Beta
@GwtIncompatible
